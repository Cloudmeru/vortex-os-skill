# image-map plugin -- map / location generation
...
$style  = if ($inputs.PSObject.Properties.Name -contains 'style') { $inputs.style } else { 'cartographic' }
$aspect = if ($inputs.PSObject.Properties.Name -contains 'aspect') { $inputs.aspect } else { '16:9' }
Write-VortexPluginLog "image-map invoked: prompt_len=$($prompt.Length) style=$style aspect=$aspect"
...
$outFile = Join-Path $outDir "map_${ts}.png"
...
Write-VortexPluginLog "image-map: produced via $($result.Provider)"
